...
class GFRunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                if self.env_name == "GoogleFootball":
                    for agent_id in range(self.num_agents):
                        idv_rews = []
                        for info in infos:
    ...
    def eval(self, total_num_steps):
        ...
        for eval_step in range(self.episode_length):
            ...
            for agent_id in range(self.num_agents):
                ...
                '''eval_action, eval_rnn_state = self.trainer[agent_id].policy.act(np.array(list(eval_obs[:, agent_id])),
                                                                                ...
                                                                                eval_masks[:, agent_id],
                                                                                deterministic=True)'''
                eval_action, eval_rnn_state = self.trainer[agent_id].policy.act(np.array(list(eval_obs)),
                                                                                eval_rnn_states[:, agent_id],
                                                                                eval_masks[:, agent_id],
                                                                                deterministic=True)
        ...
        for agent_id in range(self.num_agents):
            ...
            eval_average_episode_rewards = np.mean(np.sum(eval_episode_rewards[:, agent_id, 0], axis=0))
    ...
    def render(self):
        ...
        for episode in range(self.all_args.render_episodes):
            episode_rewards = []
            obs, _, _ = self.envs.envs[0].reset()
            ...
            for step in range(self.episode_length):
                ...
                for agent_id in range(self.num_agents):
                    ...
                    '''action, rnn_state = self.trainer[agent_id].policy.act(np.array(list(obs[:,agent_id])),
                                                                          ...
                                                                          masks[:, agent_id],
                                                                          deterministic=True)'''
                    action, rnn_state = self.trainer[agent_id].policy.act(np.array(list(obs)),
                                                                          rnn_states[0], #0
                                                                          masks[0], #0
                                                                          deterministic=True)
                ...
                # Obser reward and next obs
                actions_env_tmp = []
                for agent_id in range(len(temp_actions_env[0])):
                    for i in range(len(temp_actions_env[0][agent_id])):
                        if temp_actions_env[0][agent_id][i] == 1:
                            actions_env_tmp.append(i)

                obs, rewards, dones, infos = self.envs.step(np.array(actions_env_tmp))
                obs = obs[0]
                rewards = rewards[0]
                dones = dones[0]
            ...
            for agent_id in range(self.num_agents):
                average_episode_rewards = np.mean(np.sum(episode_rewards[:, agent_id, 0], axis=0))
